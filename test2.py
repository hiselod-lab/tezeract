...
class UniversalProductScraper:
    ...
    def _normalize_url(value: Optional[str]) -> str:
        if not value:
            return ""
        cleaned = value.strip()
        # Remove fragments/query for stable dedupe identity
    ...
    def _looks_like_homepage(url: str) -> bool:
        if not url:
            return False
        # Keep logic generic: if URL has no path segments, it's likely a root URL.
        path = re.sub(r'^https?://[^/]+', '', url)
        return path in ("", "/")

    def _build_dedup_key(self, product: Product, fallback_id: str) -> str:
        """
        Build a robust key that avoids over-collapsing products when a site has
        placeholder URLs/prices (e.g., all cards link to homepage).
        """
        name = self._normalize_text(product.name)
        price = self._normalize_price(product.price)
        image = self._normalize_url(product.image_url)
        url = self._normalize_url(product.product_url)

        # Strongest identity: a non-homepage product URL
        if url and not self._looks_like_homepage(url):
            return f"url::{url}"

        # Medium identity: semantic product signature
        if name and (price or image):
            return f"sig::{name}::{price}::{image}"

        # Weak identity: preserve records rather than dropping everything
        return f"weak::{name}::{fallback_id}"
            ...
            link_target = self.page.evaluate("""
                () => {
                    ...
                    for (const el of candidates) {
                        ...
                        if (el.matches('a[rel="next"]') && el.href) return { mode: 'goto', value: el.href };
                    }

                    // 2) semantic next controls
                    const keywords = ['next', 'older', '→', '›', '>'];
                    for (const el of candidates) {
                        if (!isVisible(el)) continue;
                        const txt = getText(el);
                        if (keywords.some(k => txt.includes(k))) {
                            if (el.tagName.toLowerCase() === 'a' && el.href) return { mode: 'goto', value: el.href };
                    ...
                    if (current) {
                        ...
                        if (target) {
                            if (target.el.tagName.toLowerCase() === 'a' && target.el.href) return { mode: 'goto', value: target.el.href };
                            return { mode: 'click', value: null };
                        }
                    }
    ...
    def scrape(self, url: str) -> ScrapingResult:
        ...
        for product, fallback_id in all_products_with_id:
            key = self._build_dedup_key(product, fallback_id)
