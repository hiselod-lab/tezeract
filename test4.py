...
class Product(BaseModel):
    ...
    #new change - add sale price fields
...
class UniversalProductScraper:
    ...
    def detect_product_containers(self) -> List[str]:
        ...
        container_xpaths = self.page.evaluate("""
            () => {
                ...
                //new change - Strategy 1: Look for data-product-id or similar attributes
                const productsWithDataAttrs = Array.from(document.querySelectorAll('[data-product-id], [data-product], [data-item-id], [data-sku]'));
                if (productsWithDataAttrs.length >= 2) {
                    console.log('Found products via data attributes:', productsWithDataAttrs.length);
                    return productsWithDataAttrs.map(el => getXPath(el)).filter(xpath => xpath !== null);
                }
                
                //new change - Strategy 2: Look for repeated link patterns (product cards usually link to product pages)
                const allLinks = Array.from(document.querySelectorAll('a[href*="/product"], a[href*="/item"], a[href*="/p/"], a[href*="/collections/"]'));
                const linkParents = allLinks.map(link => {
                    let parent = link.parentElement;
                    let depth = 0;
                    while (parent && depth < 5) {
                        const hasImage = parent.querySelector('img') !== null;
                        const hasPrice = /[$£€₹]\s*[\d,]+/.test(parent.innerText || '');
                ...
                if (linkParents.length >= 2) {
                    console.log('Found products via link patterns:', linkParents.length);
                    const uniqueParents = [...new Set(linkParents)];
                    return uniqueParents.map(el => getXPath(el)).filter(xpath => xpath !== null);
                }
                
                //new change - Strategy 3: Original structural pattern recognition (with improvements)
                ...
                candidates.forEach(el => {
                    ...
                    //new change - Add sibling count to fingerprint for better grouping
                    const siblingCount = el.parentNode ? el.parentNode.children.length : 0;
                    
                    //new change - Improved fingerprint with sibling context
                    const fingerprint = `${tagName}|depth:${depth}|siblings:${siblingCount}`;
                ...
                for (const [fp, elements] of fingerprints.entries()) {
                    //new change - Lower minimum from 3 to 2 for small catalogs
                    if (elements.length >= 2 && elements.length > largestGroup.length) {
        ...
        try:
            product_data = self.page.evaluate(f"""
                (xpath) => {{
                    ...
                    //new change - Extract BOTH regular and sale prices
                    const getPrices = () => {{
                        const allText = container.innerText || '';
                        ...
                        const patterns = [
                            ...
                            /Rs\\.?\\s*([\\d,]+(?:\\.\\d{{2}})?)/gi,
                            /₹\\s*([\\d,]+(?:\\.\\d{{2}})?)/gi,
                            /([\\d,]+\\.\\d{{2}})\\s*(?:USD|GBP|EUR|INR)/gi,
                            /\\b([\\d,]+)\\s*(?:USD|GBP|EUR|Rs|PKR)\\b/gi
                        ];
                        ...
                        for (const pattern of patterns) {{
                            ...
                            for (const match of matches) {{
                                foundPrices.push(match[0].trim());
                            }}
                        }}
                        
                        //new change - Deduplicate and parse prices
                        const uniquePrices = [...new Set(foundPrices)];
                        
                        if (uniquePrices.length === 0) {{
                            return {{ price: null, original_price: null, sale_price: null }};
                        }}
                        
                        //new change - If multiple prices, identify sale scenario
                        if (uniquePrices.length === 1) {{
                            return {{ price: uniquePrices[0], original_price: null, sale_price: null }};
                        }}
                        
                        //new change - Parse numeric values to identify original vs sale
                        const parsePrice = (priceStr) => {{
                            const cleaned = priceStr.replace(/[^\\d.]/g, '');
                            return parseFloat(cleaned) || 0;
                        }};
                        
                        const priceValues = uniquePrices.map(p => ({{ str: p, val: parsePrice(p) }}));
                        priceValues.sort((a, b) => b.val - a.val);
                        
                        //new change - Higher price = original, lower = sale
                        const original = priceValues[0].str;
                        const sale = priceValues[priceValues.length - 1].str;
                        
                        //new change - Check for sale indicators in text
                        const hasSaleKeywords = /was|now|sale|save|off|discount/i.test(allText);
                        
                        if (hasSaleKeywords && priceValues.length >= 2) {{
                            return {{ price: sale, original_price: original, sale_price: sale }};
                        }}
                        
                        //new change - Default: use first found price
                        return {{ price: uniquePrices[0], original_price: null, sale_price: null }};
                    }};
                    ...
                    //new change - Call getPrices instead of getPrice
    ...
    def detect_pagination_method(self) -> Optional[str]:
        ...
        4. Numbered pages - are there clickable page numbers?
        
        Returns: 'scroll', 'button', 'url', 'numbered', or None
        """
        ...
        #new change - TEST 3: NUMBERED PAGES
        print("  Testing for numbered pagination...")
        numbered_exists = self.page.evaluate("""
            () => {
                const links = Array.from(document.querySelectorAll('a, button'));
                const numberedLinks = links.filter(link => {
                    const text = (link.innerText || '').trim();
                    return /^[0-9]+$/.test(text) && parseInt(text) >= 2;
                });
                return numberedLinks.length >= 2;
            }
        """)
        
        if numbered_exists:
            print("  ✅ NUMBERED pagination detected")
            return 'numbered'
    ...
    def execute_dynamic_pagination(self, max_pages: int = 30) -> None:
        ...
        for i in range(max_pages):
            ...
            #new change - Track URL to detect actual navigation
            before_url = self.page.url
            
            #new change - Try numbered pagination first (for sites like nehera.com)
            next_page_link = self.page.evaluate("""
                () => {
                    const links = Array.from(document.querySelectorAll('a, button'));
                    ...
                    if (activeLink) {
                        const activeText = activeLink.innerText.trim();
                        if (/^[0-9]+$/.test(activeText)) {
                            currentPage = parseInt(activeText);
                        }
                    }
                    ...
                    if (nextPageLink && nextPageLink.offsetParent !== null) {
                        return nextPageLink.href || 'click';
                    }
            ...
            if next_page_link:
                print(f"  📄 Clicking next page number...")
                #new change - If it's a link with href, use goto for proper navigation
                if next_page_link != 'click' and next_page_link.startswith('http'):
                    try:
                        self.page.goto(next_page_link, wait_until="domcontentloaded", timeout=self.timeout)
                        self.page.wait_for_load_state("networkidle", timeout=15000)
                        time.sleep(2)
                    except PlaywrightTimeout:
                        print("  ⚠️  Page navigation timeout, continuing...")
                        time.sleep(3)
                else:
                    #new change - For button-based pagination, click and wait
                ...
                #new change - Verify page actually changed by checking URL
                after_url = self.page.url
                if after_url == before_url:
                    #new change - URL didn't change, might be AJAX pagination - wait longer
                    time.sleep(2)
                ...
                if new_count > before_count:
                    ...
                    continue
                #new change - Also continue if URL changed (even if count is same due to detection issues)
                elif after_url != before_url:
                    print(f"  Page {i + 1}: URL changed, continuing...")
                    continue
                else:
    ...
    def scrape(self, url: str) -> ScrapingResult:
        ...
        #new change - Improved deduplication: use product_url as primary key
        print(f"\n🧹 Deduplicating products...")
        unique_products = {}
        duplicates_dropped = 0
        
        for product in all_products:
            #new change - Use product_url as primary key (most reliable unique identifier)
            #new change - Fallback to name+price if URL is missing
...
def main():
    ...
    # URLs to scrape
    test_urls = [
        "https://nehera.com/collections/spring-sale"
    ]
